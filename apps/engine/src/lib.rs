...
struct AnalysisResult {
    win_probability: f64,
    best_move: [u8; 2],
    target_pip: Option<u8>, // Tells Next.js which side of the board to use
    depth: u32,
}
...
pub fn analyze_board_state(board_json: &str) -> String {
    ...
    let (best_action, win_probability) = engine.search(&state);

    let result = AnalysisResult {
        win_probability,
        best_move: [best_action.tile.high, best_action.tile.low],
        target_pip: best_action.target_pip,

  ...
  return (
    <div className="mx-auto max-w-6xl px-6 py-16 md:py-32">
      ...
      <div className="grid grid-cols-1 gap-10 md:grid-cols-2 lg:grid-cols-3">
        {blogPosts.length === 0 ? (
          <p className="text-foreground/70 col-span-full text-xl font-bold uppercase tracking-widest">
          ...
          blogPosts.map((post) => (
            <Link key={post.id} href={`/blog/${post.slug}`} className="group h-full">
              <article className="bg-card border-border flex h-full flex-col border-4 shadow-[8px_8px_0px_0px_var(--brutalist-shadow)] transition-all duration-200 hover:-translate-x-1 hover:-translate-y-1 hover:shadow-[12px_12px_0px_0px_var(--brutalist-shadow)]">
                <div className="flex flex-1 flex-col p-8">
                  {/* Brutalist Author Block mapped to semantic foreground/background tokens */}
                  <div className="border-border mb-8 flex items-center gap-4 border-b-2 pb-6">
                  ...
                  <h2 className="text-foreground group-hover:text-primary mb-6 line-clamp-3 text-2xl font-bold uppercase leading-tight tracking-wide transition-colors">
                    {isAr ? post.title_ar : post.title_en}
                  </h2>

                  <div className="text-primary mt-auto flex items-center pt-6 text-sm font-bold uppercase tracking-widest transition-transform group-hover:translate-x-2 rtl:group-hover:-translate-x-2">

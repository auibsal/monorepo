...
export default function KanbanBoard() {
  ...
  if (loading) {
    return (
      <div className="p-8 font-bold uppercase tracking-widest text-foreground/50 flex items-center justify-center h-64 border-4 border-dashed border-border/20">
      ...
      <div className="flex gap-8 overflow-x-auto pb-8 items-start snap-x">
        {STATUSES.map(status => (
          <div key={status.id} className="w-80 flex-shrink-0 flex flex-col snap-start">
            ...
            <h3 className="font-bold text-foreground mb-4 uppercase tracking-widest flex items-center justify-between border-b-4 border-border pb-3">
              {status.label}
              <span className="bg-foreground text-background py-1 px-3 font-bold text-xs shadow-[4px_4px_0px_0px_var(--primary)]">
              ...
              {(provided, snapshot) => (
                <div
                  ...
                  className={`min-h-[600px] border-4 flex flex-col gap-5 p-4 transition-colors ${
                    snapshot.isDraggingOver ? 'bg-foreground/5 border-primary border-dashed' : 'bg-transparent border-border'
                  }`}
                >
                      ...
                      <Draggable key={sub.id} draggableId={sub.id} index={index}>
                        {(provided, snapshot) => (
                          <div
                            ...
                            className={`bg-card text-foreground p-5 border-4 transition-all ${
                              snapshot.isDragging 
                                ? 'border-primary shadow-[12px_12px_0px_0px_var(--primary)] -rotate-2 z-50 scale-105' 
                                : 'border-border shadow-[6px_6px_0px_0px_var(--brutalist-shadow)] hover:shadow-[8px_8px_0px_0px_var(--brutalist-shadow)] hover:-translate-y-0.5'
                            }`}
                            style={{ ...provided.draggableProps.style }}
                          >
                            <div className="flex items-start gap-3">
                              <GripVertical className="text-foreground/30 flex-shrink-0 mt-0.5 cursor-grab active:cursor-grabbing" size={20} />
                              <div className="flex-1 min-w-0">
                                {/* ⚡ Bolt Routing Optimization: Locked the href to the secure perimeter */}
                                <Link href={`/editorial/submissions/${sub.id}`} className="block group">
                                  <h4 className="font-bold uppercase tracking-wide mb-3 text-foreground group-hover:text-primary transition-colors truncate leading-tight">
                                    {sub.title}
                                  </h4>
                                </Link>
                                <div className="flex justify-between items-end mt-5">
                                  <span className="text-xs font-bold uppercase tracking-widest bg-foreground text-background px-3 py-1.5 border-2 border-border shadow-[2px_2px_0px_0px_var(--brutalist-shadow)]">
                                    {sub.type}
                                  </span>

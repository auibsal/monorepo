...
export default function GradingPage() {
  ...
  const fetchDossierData = useCallback(async () => {
    ...
    try {
      // ⚡ Bolt Optimization: Use Promise.all to fetch submission data and editor roster concurrently, eliminating a network waterfall
      const [subRes, editorRes] = await Promise.all([
        supabase
          .from('submissions')
      ...
      const { data: subData, error: subError } = subRes;
      const { data: editorData, error: editorError } = editorRes;

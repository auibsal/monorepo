...
export default function GradingPage() {
  ...
  const fetchDossierData = useCallback(async () => {
    ...
    try {
      // ⚡ Bolt Optimization: Use Promise.all to fetch submission and editors concurrently
      const [submissionResponse, editorsResponse] = await Promise.all([
        supabase
          .from('submissions')
      ...
      const { data: subData, error: subError } = submissionResponse;
      const { data: editorData, error: editorError } = editorsResponse;

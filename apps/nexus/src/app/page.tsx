...
export default async function NexusHome() {
  ...
  if (isEditor) {
    const [pendingRes, membersRes, eventsRes] = await Promise.all([
      supabase
        .from('submissions')
        // ⚡ Bolt Optimization: Use .select('id') instead of .select('*') for count queries to reduce PostgreSQL index evaluation time and prevent PostgREST from parsing full row metadata.

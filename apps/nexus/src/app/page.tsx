...
export default async function NexusHome() {
  ...
  if (isEditor) {
    // ⚡ Bolt Optimization: Using .select('id') instead of .select('*') for count-only queries prevents PostgREST from parsing full row metadata, reducing index evaluation time.
